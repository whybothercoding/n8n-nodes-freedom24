...
- **Known, unresolved portal finding:** `methods/credentialTest.ts` uses `this.helpers.request` (deprecated per `no-deprecated-workflow-functions`). Confirmed via `n8n-core`'s own `CredentialTestContext` implementation (checked against the latest published `n8n-core@2.16.1`) that `ICredentialTestFunctions.helpers` is constructed with *only* SSH-tunnel functions and `request` — no `httpRequest`, in any current n8n version. There is no non-deprecated way to make an HTTP call from inside a `testedBy` credential-test function. A declarative `test:` property would avoid the deprecated call, but can't replicate this test's actual checks (Tradernet returns HTTP 200 even on failed auth; the failure is in the JSON body or a missing `Set-Cookie` header, and `ICredentialTestRequest.rules` can only match a status code or a body key/value) without regressing to the "always passes" bug 0.2.0 fixed by adding real `testedBy` functions in the first place. Left as-is with a justifying inline comment; this is a genuine platform gap to raise with the portal reviewer, not something to code around.
