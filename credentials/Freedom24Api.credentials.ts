...
export class Freedom24Api implements ICredentialType {
	...
	// 2026-08-26: a bad public key or bad signature both come back as a real HTTP 403 with a
	// `{"error": "..."}` body — not Tradernet's usual "200 with the failure in the body" shape —
	// so the default non-2xx check below is a genuine, not just declarative-for-its-own-sake, test.
	async authenticate(
