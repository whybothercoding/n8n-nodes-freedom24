...
- **Credential testing splits by credential type, confirmed against the live API 2026-08-26 (not assumed):** `Freedom24Api` (API Key) uses a declarative `test:` + function-based `authenticate` on `Freedom24Api.credentials.ts` (reusing `signPayload`/`buildApiKeyHeaders` unchanged) — a bad public key or bad signature both come back as a real HTTP 403 with `{"error": "..."}`, not Tradernet's usual 200-with-error-body shape, so the default non-2xx check is a genuine test. `Freedom24UserApi` (User Login) stays on the custom `methods.credentialTest.freedom24UserApiCredentialTest` (`testedBy`): a deliberately wrong login/password still comes back HTTP 200 with `{"error": "Incorrect email or password"}`, and Tradernet issues a session cookie either way — so status code and cookie presence are both useless, and only body inspection catches it, which `ICredentialTestRequest.rules` (status code or exact body key/value) can't express. That function is the one remaining `no-deprecated-workflow-functions` portal finding: confirmed via `n8n-core`'s own `CredentialTestContext` implementation (checked against the latest published `n8n-core@2.16.1`) that `ICredentialTestFunctions.helpers` is constructed with *only* SSH-tunnel functions and `request` — no `httpRequest`, in any current n8n version. There is no non-deprecated way to make an HTTP call from inside a `testedBy` credential-test function, and switching this one credential to declarative would regress to the "always passes" bug 0.2.0 fixed by adding real `testedBy` functions in the first place. Left as-is with a justifying inline comment; this is a genuine platform gap to raise with the portal reviewer, not something to code around.
